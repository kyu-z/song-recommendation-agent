...
class DiscoveryChain:
    ...
    def _find_authority_links(self, context: Dict[str, Any]) -> List[str]:
        """Stage 1: Find authority music platform links"""
        refined_query = context.get('refined_query', '')
        native_name = context.get('native_name')
        origin_region = context.get('origin_region', 'unknown')
        search_strategy = context.get('search_strategy', 'international')
        
        print(f"🎯 [权威链接搜索] 地域: {origin_region}, 策略: {search_strategy}")
        
        # Build authority search queries
        authority_queries = []
        ...
        authority_queries.extend([
            f"site:music.apple.com {base_query}",
            f"site:open.spotify.com {base_query}",
            f"site:wikipedia.org {base_query} discography",
        ])
        
        # Region-specific authority sites
        if origin_region == 'Japan':
            authority_queries.extend([
                f"site:oricon.co.jp {base_query}",
                f"site:natalie.mu {base_query}",
            ])
        elif origin_region == 'Korea':
            authority_queries.extend([
                f"site:melon.com {base_query}",
                f"site:soompi.com {base_query}",
            ])
        elif origin_region == 'Greater China':
            authority_queries.extend([
                f"site:kkbox.com {base_query}",
                f"site:xiami.com {base_query}",
            ])
        
        print(f"🎯 [权威查询] {len(authority_queries)} 个权威站点查询")
        
        # Execute authority searches
        authority_links = []
        for query in authority_queries[:3]:  # Limit to 3 to avoid rate limiting
            try:
                print(f"🔗 [权威搜索]: {query}")
                results = self.brave_search.run(query)
                ...
                print(f"⚠️  Authority search failed: {e}")
        
        print(f"🔗 [权威链接] 找到 {len(authority_links)} 个权威链接")
        return authority_links[:5]  # Limit to top 5
    
    def _extract_authority_links(self, search_results: str) -> List[str]:
        """Extract authority platform URLs from search results"""
        authority_domains = [
            'music.apple.com', 'open.spotify.com', 'wikipedia.org',
            'oricon.co.jp', 'natalie.mu', 'melon.com', 'soompi.com',
            'kkbox.com', 'xiami.com'
        ]
    ...
    def _extract_songs_from_text(self, search_results: List[str], context: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if json_match:
                print(f"🎯 [调试] 找到JSON匹配: {json_match.group()}")
                ...
                for song in extracted_songs:
                    if isinstance(song, dict) and song.get('song') and song.get('artist'):
                        ...
                        if enhanced_song['explanation']:
                            print(f"🎯 [元数据校验] {enhanced_song['song']}: {enhanced_song['explanation']}")
